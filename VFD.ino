...
void VFD::renderMeter(Meter* meter)
{
  switch (meter->getType())
  {
    ...
    case Meter::_int2:
      if (usd)
        data[meter->getpos() + 8] = bts(meter->getMeasurement() % 100);
      else
        data[(1 - meter->getpos()) + 8] = transpose(bts(meter->getMeasurement() % 100), trn);
      break;
    ...
    case Meter::_int4:
      if (usd)
      {
        ...
        data[9] = data[9]| ~0x00C0&bts(meter->getMeasurement() % 100);
      }
      else
      {
        data[9] = data[9]| ~0x00C0&transpose(bts((meter->getMeasurement() / 100) % 100), trn);
    ...
    case Meter::_intN:
      ...
      for (int i = 0; i < meter->getDigits(); i++)
      {
        if (i + meter->getpos() < 8)
        {
          if (usd)
          {
            data[meter->getpos()+i] = data[meter->getpos()+i]| ~0x00E0&anch((meterCopy / (int)pow(10.0, i * 1.0)) % 10 + '0');
          }
          else
          {
            data[8- i - meter->getpos()] = data[8- i - meter->getpos()] | ~0x00E0&transpose(anch((meterCopy / (int)pow(10.0, i * 1.0)) % 10 + '0'), tra);
...
{
  for (byte i = 0; contents[i] && (i < 8); i++)
  {
    vfd->setChar(i, contents[i]);
  }
}
